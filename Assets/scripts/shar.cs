...
public class shar : MonoBehaviour
{
    ...
    public TMP_Text tikpultext,foiztext,pultext,bwtext;
    ...
    void Start()
    {
        ...
        muziqa.volume=.7f;
        first=pul;
        if(File.ReadAllText(Application.persistentDataPath + "/muziqa1.txt")=="0"){
            ovozyoq();
        }else{
            ovozoch();
        }
        if(File.ReadAllText(Application.persistentDataPath + "/muziqa.txt")=="0"){
            muzyoq();
        }else{
            muzoch();
        }
